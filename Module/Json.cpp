...
Value::Value(const Value& v) : mValueType(v.mValueType)
{
	switch (mValueType)
	{
		...
		case IntVal			: mIntVal = v.mIntVal; mInt64Val = (__int64)v.mInt64Val; mFloatVal = (float)v.mIntVal; mDoubleVal = (double)v.mIntVal; break;
		case Int64Val		: mInt64Val = v.mInt64Val; mIntVal = (int)v.mInt64Val; mFloatVal = (float)v.mInt64Val; mDoubleVal = (double)v.mInt64Val; break;
		case FloatVal		: mFloatVal = v.mFloatVal; mIntVal = (int)v.mFloatVal; mInt64Val = (__int64)v.mFloatVal; mDoubleVal = (double)v.mFloatVal; break;
		case DoubleVal		: mDoubleVal = v.mDoubleVal; mIntVal = (int)v.mDoubleVal; mInt64Val = (__int64)v.mDoubleVal; mFloatVal = (float)v.mDoubleVal; break;
...
Value& Value::operator =(const Value& v)
{
	...
	{
		case StringVal		: mStringVal = v.mStringVal; break;
		case IntVal			: mIntVal = v.mIntVal; mInt64Val = (__int64)v.mIntVal; mFloatVal = (float)v.mIntVal; mDoubleVal = (double)v.mIntVal; break;
		case Int64Val		: mInt64Val = v.mInt64Val; mIntVal = (int)v.mInt64Val; mFloatVal = (float)v.mInt64Val; mDoubleVal = (double)v.mInt64Val; break;
		case FloatVal		: mFloatVal = v.mFloatVal; mIntVal = (int)v.mFloatVal; mInt64Val = (__int64)v.mFloatVal; mDoubleVal = (double)v.mFloatVal; break;
		case DoubleVal		: mDoubleVal = v.mDoubleVal; mIntVal = (int)v.mDoubleVal; mInt64Val = (__int64)v.mDoubleVal; mFloatVal = (float)v.mDoubleVal; break;
		case BoolVal		: mBoolVal = v.mBoolVal; break;
...
std::string SerializeValue(const Value& v)
{
	...
	switch (v.GetType())
	{
		...
		case Int64Val		: snprintf(buff, BUFF_SZ, "%lld", (__int64)v); str = buff; break;
		...
		case StringVal:
		{
			//str = std::string("\"") + string_replace((std::string)v, "\"", "\\\"") + std::string("\"");
			std::string strV = v.ToString();
			string_replace(strV, "\\", "\\\\");
			str = std::string("\"") + string_replace(strV, "\"", "\\\"") + std::string("\"");
			break;
		}
	}
...
static size_t GetEndOfArrayOrObj(const std::string& str, std::stack<StackDepthType>& depth_stack)
{
	...
	for (; i < str.length(); i++)
	{
		if (str[i] == '\"')
		{
			...
			in_quote = bIsChar ? !in_quote : in_quote;
		}
		else if (!in_quote)
		{
			if (str[i] == '[')
...
static Value DeserializeValue(std::string& str, bool* had_error, std::stack<StackDepthType>& depth_stack)
{
	...
	*had_error = false;
	...
	if (str[0] == '[')
	{
		...
		int nEnd = array_str.rfind(']');
		array_str.erase(nEnd + 1);
	...
	else if (str[0] == '{')
	{
		...
		int nEnd = obj_str.rfind('}');
		obj_str.erase(nEnd + 1);
